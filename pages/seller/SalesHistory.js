  ...
  script() {
    const initialize = () => {
      ...
      const products = rawProducts.map((product) => ({
        ...
        Stock: (product.stock || []).reduce((total, stockItem) => {
          return (
            total +
            (stockItem.sizes || []).reduce(
              (sum, size) => sum + (size.qty || 0),
              0
            )
          );
        }, 0),
      ...
      const salesData = products.map((product) => {
        ...
        const lastSold = productOrders.length
          ? productOrders[0].orderDate || "Not Available"
          : "Not Available";
        return {
          name: product.Name,
          sold: totalSold,
      ...
      document.getElementById(
        "bestSeller"
      ).textContent = `Best Seller: ${bestSeller}`;
      document.getElementById(
        "leastSeller"
      ).textContent = `Least Seller: ${leastSeller}`;
      document.getElementById(
        "totalSales"
      ).textContent = `Total Sales: ${totalSales} items`;
